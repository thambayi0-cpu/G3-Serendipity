...
void repRetail(bookNode*& head, bookNode*& tail)
{
	...
	while (currentPage <= totalPage)
	{
		...
		cout << "│ " << setw(TITLE_WIDTH) << "TITLE" << " "
			  ...
			  << setw(QTY_WIDTH) << "QTY" << " "
			  << setw(RETAIL_WIDTH) << "RETAIL" << right << setw(TOTAL_WIDTH-65) << " │" << endl;
		cout << left;
		...
		for (int i = 0; i < BOOKS_SHOWN; i++)
		{
			...
			cout << "│ " << setw(TITLE_WIDTH) << ((current->book->getTitle().length() > TITLE_WIDTH) ? current->book->getTitle().substr(0, TITLE_WIDTH-1) + "…" : current->book->getTitle()) << " "
			     ...
			     << setprecision(2) << "$ " << setw(RETAIL_WIDTH-2) << current->book->getRetail() << right
		...
		{
			cout << "│ Total Retail Inventory Value: $ " << setw(TOTAL_WIDTH-36) << total << "│" << endl;
